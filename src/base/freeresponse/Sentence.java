...
public class Sentence {
    ...
    public int findNthTime(String str, int n) {
        if (!currSent.contains(str)) return -1;
        int lastIndex = currSent.indexOf(str);
        for(int i = 2; i <= n; i++){
            lastIndex = currSent.indexOf(str, lastIndex + str.length());
    ...
    public void replaceNthTime(String str, int n, String repl) {
        ...
        int replaceIndex = this.findNthTime(str, n);
        if (replaceIndex != -1){
            currSent = currSent.substring(0, replaceIndex) + repl + currSent.substring(replaceIndex + str.length());
        }
    }
